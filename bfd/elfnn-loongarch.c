...
loongarch_elf_grok_prstatus (bfd *abfd, Elf_Internal_Note *note)
{
  switch (note->descsz)
    ...
    case sizeof(prstatus_t):  /* sizeof(struct elf_prstatus) on Linux/Loongarch.  */
      /* pr_cursig */
  ...
  return _bfd_elfcore_make_pseudosection (abfd, ".reg", sizeof(elf_gregset_t),
					  note->descpos + offsetof(prstatus_t, pr_reg));
}
...
loongarch_elf_grok_psinfo (bfd *abfd, Elf_Internal_Note *note)
{
  switch (note->descsz)
    ...
    case sizeof(prpsinfo_t): /* sizeof(struct elf_prpsinfo) on Linux/Loongarch.  */
      /* pr_pid */
